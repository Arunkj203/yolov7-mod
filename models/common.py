...
class DC(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        self.deformable_conv = DeformConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding)
...
class ELAN(nn.Module):
    def __init__(self, in_channels, out_channels=256, mid_channels=64, expansion=0.5):
        ...
        self.conv2 = Conv(mid_channels, mid_channels, 3, 1)

        # Concatenation
        self.concat_conv = Conv(4 * mid_channels, out_channels, 1, 1)
        # print(f"ELAN - in,mid,out - {in_channels, mid_channels, out_channels}")
    ...
    def forward(self, x):
        ...
        # Concatenate the outputs from different layers
        out = torch.cat([x6, x4, x2, x1], dim=1)
        final_op = self.concat_conv(out)
...
class ELAN_H(nn.Module):
    def __init__(self, in_channels, out_channels=512, mid_channels=256, expansion=0.5):
        ...
        # Following conv layers
        self.conv2 = Conv(out_channels, mid_channels, 3, 1)
    ...
    def forward(self, x):
        ...
        out = torch.cat([x6, x5, x4, x3, x2, x1], dim=1)
...
class MP1(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=2, stride=2):
        ...
        # print(f"MP1-{in_channels,out_channels}")
        out_channels //=2
        self.maxpool = nn.MaxPool2d(kernel_size=kernel_size, stride=stride)
        self.branch1_conv1 = Conv(in_channels, out_channels, 1, 1)

        self.branch1_conv2 = Conv(out_channels, out_channels, 3, 2)
    ...
    def forward(self, x):
        ...
        # print("finish", self.in_ch)
        final_op = torch.cat([x1, x2], dim=1)
